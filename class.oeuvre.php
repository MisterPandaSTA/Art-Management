...
class Oeuvre {
 ...
 /*  constructeur */
    ...
    function __construct($id=0){
        if($id!=0){
            $res=sql("SELECT img_name,nom_artiste, artiste.description, oeuvre.nom, type_oeuvre, oeuvre.id_artiste, id_oeuvre, dimensions, poids, description_oeuvre, date_creation, livraison  FROM oeuvre INNER JOIN artiste ON oeuvre.id_artiste=artiste.id_artiste  WHERE id_oeuvre='".$id."'");
            /*var_dump($res);*/
            $user=$res[0];
    ...
    function getIdOeuvreNext(){
        
        return $this->id_oeuvre+1;
    }
   ...
   function syncDb() {
    ...
    function affichage(){
        $requete = htmlspecialchars($_POST['requete']);
         $res=sql("SELECT * FROM oeuvre WHERE nom LIKE '%$requete%' ORDER BY id_oeuvre DESC");
 ...
 /***** FONCTION SELECT  *****/
    function select (){
    $res=sql("SELECT artiste.nom as nom_artiste, oeuvre.nom, type_oeuvre, oeuvre.id_artiste, id_oeuvre, dimensions, poids, description_oeuvre, date_creation, livraison  FROM oeuvre LEFT JOIN artiste ON oeuvre.id_artiste=artiste.id_artiste  ");
        foreach ($res as $user){
            echo "<tr>";
    ...
    /*** creation de la fonction de recherche pour le front **/


    function recherche(){
        if(isset($_POST['requete']) && $_POST['requete'] != NULL){
        $requete = htmlspecialchars($_POST['requete']);
         $res=sql("SELECT img_name, nom_artiste, nom, theme, id_oeuvre FROM oeuvre LEFT JOIN artiste ON oeuvre.id_artiste=artiste.id_artiste LEFT JOIN exposition ON oeuvre.id_artiste=exposition.id_artiste  WHERE (nom   LIKE '%$requete%' OR  nom_artiste  LIKE '%$requete%' OR theme LIKE '%$requete%' ) ORDER BY id_oeuvre DESC");
         foreach ($res as $value) {
             /*var_dump($value);
    */  echo '<div class="col-md-3 margin_img"><a href="rechercher.php?id_oeuvre='.$value['id_oeuvre'].'"><img src="img/'.$value['img_name'].'"/></a></div>';
            }
        }
    }

...
class Exposition {
  ...
  private $nom_artiste;
    ...
    function __construct($id=0){
        if($id!=0){
            $res=sql("SELECT artiste.nom as nom_artiste, id_exposition, exposition.id_artiste, date_debut, date_fin, theme FROM exposition INNER JOIN artiste ON exposition.id_artiste = artiste.id_artiste WHERE id_exposition='".$id."'");
            $user=$res[0];
    ...
    function syncDb() {
        if(empty($this->id_exposition)){
            ...
            $res= sql("INSERT INTO exposition (id_exposition,id_artiste,date_debut,date_fin,theme)
                      VALUES (NULL,'".$this->id_artiste."','".$this->date_debut."','".$this->date_fin."','".$this->theme."')");
            ...
            //Sinon on fait un UPDATE
                ...
                id_exposition = '".$this->id_exposition."',
                id_artiste= '".$this->id_artiste."',
                date_debut= '".$this->date_debut."',
                date_fin= '".$this->date_fin."',
                theme = '".addslashes($this->theme)."'
                WHERE id_exposition = '".$this->id_exposition."'");
    ...
    */  echo ''.$value['theme'].'<br><a href="modifier.php?id_exposition='.$value['id_exposition'].'">Modifier </a><a href="delete.php?id_exposition='.$value['id_exposition'].'">Supprimer </a><a href="description.php?id_exposition='.$value['id_exposition'].'">Description</a><br><br>';
    ...
    function delete(){
            $res=sql("DELETE FROM exposition WHERE id_exposition='".$_GET['id_exposition']."'");
    }
